...
const Login = () => {
    ...
    try {
        ...
        if (state === 'Sign Up') {
            ...
            if (data.success) {
                ...
                toast.error(data.message)  // ✅ data.message not error.message
            }
        }
    } catch (error) {
  ...
  return (
    <div className='flex items-center justify-center min-h-screen px-6 sm:px-0 bg-gradient-to-br from-blue-300 to-green-400'>
      <img onClick={()=>navigate('/')} src={assets.logo} alt="" className='absolute left-5 sm:left-20 top-5 w-28 sm:w-32 cursor-pointer'/>
      <div className='bg-slate-900 p-10 rounded-lg shadow-lg w-full sm:w-96 text-indigo-300 text-sm'>
      <h2 className='text-3xl font-semibold text-white text-center mb-3'>{state==='Sign Up' ? 'Create Account':'Login'}</h2>
      <p className='flex items-center justify-center py-2 pb-6'>{state==='Sign Up' ? 'Create your account':'Login to your account!'}</p>
      ...
      </p>):(<p className='text-gray-400 text-center text-xs mt-4'>Don't have a account?{' '}
